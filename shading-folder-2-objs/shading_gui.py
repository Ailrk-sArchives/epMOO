    ...
    def init(self):
        ...
        with flx.FormLayout(flex=1) as self.other_constants_model:
            self.floor_height = flx.LineEdit(
                title='Floor Height', text='{}'.format(float(self.root.floor_height_float)))
            self.window_height = flx.LineEdit(
                title='Window Height', text='{}'.format(float(self.root.window_height_float)))
            self.window_edge_height = flx.LineEdit(title='Window edge height', text='{}'.format(
                float(self.root.window_edge_height_float)))
            self.heating_setpoint = flx.LineEdit(title='Heating setpoint', text='{}'.format(
                float(self.root.heating_setpoint_float)))
            self.cooling_setpoint = flx.LineEdit(title='Cooling setpoint', text='{}'.format(
                float(self.root.cooling_setpoint_float)))
            self.apmv_upper = flx.LineEdit(
                title='Apmv upper', text='{}'.format(float(self.root.apmv_upper_float)))
            self.apmv_lower = flx.LineEdit(
                title='Apmv lower', text='{}'.format(float(self.root.apmv_lower_float)))
